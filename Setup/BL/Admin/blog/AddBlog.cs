...
namespace Setup.BL.Admin.blog
{
    public class AddBlog : IAddBlog
    {
        ...
        public AddBlog(IConfiguration config, IWebHostEnvironment environment)
        {
        ...
        public CommonResponse<AddBlogResponse> InsertBlog(AddBlogRequest objRequest)
        {
            CommonResponse<AddBlogResponse> response = new CommonResponse<AddBlogResponse>();
            ...
            try
            {
                if (objRequest.Image != null && objRequest.Image.Length > 0)
                {
                    ...
                    if (allowdedExtensions.Contains(fileExtension.ToLower()))
                    {
                        ...
                        dataAccess.ExecuteQuery(CommandType.StoredProcedure, "InsertBlog", ds, TableName, parameters);
