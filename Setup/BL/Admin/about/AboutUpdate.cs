...
{
    public class AboutUpdate : IAboutUpdate
    {
        private readonly IConfiguration _configuration;
        private readonly IWebHostEnvironment _environment;
        private readonly DataAccessClass dataAccess;

        public AboutUpdate(IConfiguration config, IWebHostEnvironment environment)
        {
            _configuration = config;
            _environment = environment;
            dataAccess = new DataAccessClass(_configuration);
        }
        public CommonResponse<AboutUpdateResponse> UpdateAbout(AboutUpdateRequest objRequest)
        {
            CommonResponse<AboutUpdateResponse> response = new CommonResponse<AboutUpdateResponse>();
            try
            {
                if (objRequest.Image != null && objRequest.Image.Length > 0)
                {
                    ...
                    if (allowdedExtensions.Contains(fileExtension.ToLower()))
                    {
                        ...
                        dataAccess.ExecuteQuery(CommandType.StoredProcedure, "UpdateAbout", ds, TableName, parameters);
