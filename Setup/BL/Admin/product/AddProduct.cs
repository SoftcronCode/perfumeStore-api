...
namespace Setup.BL.Admin.product
{
    public class AddProduct : IAddProduct
    {
        ...
        public AddProduct(IConfiguration config, IWebHostEnvironment environment)
        {
        ...
        {
            CommonResponse<AddProductResponse> response = new CommonResponse<AddProductResponse>();
            ...
            try
            {
                if (objRequest.Image != null && objRequest.Image.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        MySqlParameter[] parameters =
                        {
                                ...
                                new MySqlParameter("@SPImage", MySqlDbType.String) {Value = ImageNames }
                            };
                        #endregion

                        DataSet ds = new DataSet();
                        string[] TableName = { "Response" };
                        dataAccess.ExecuteQuery(CommandType.StoredProcedure, "InsertProduct", ds, TableName, parameters);
                ...
                {
                    response.ResponseCode = 0;
                    response.ResponseMessage = "Please Select Image First.";
                    response.ResponseData = null;
                }
            }
            catch (Exception ex)
