...
using Setup.Response.Admin.product;
using System;
...
namespace Setup.BL.Admin.product
{
    public class GetProduct : IGetProduct
    {
        ...
        public GetProduct(IConfiguration configuration)
        {
            _configuration = configuration;
            _dataAccessClass = new DataAccessClass(_configuration);
        }

        #region Get Product Logic
        public CommonResponse<GetProductResponse> FetchProduct(GetProductRequest objRequest)
        {
            CommonResponse<GetProductResponse> response = new CommonResponse<GetProductResponse>();
            ...
            try
            {
                ...
                _dataAccessClass.ExecuteQuery(CommandType.StoredProcedure, "FetchProductForAdmin", ds, TableName, parameters);
        ...
        #region Get Product images By Id Logic
        public CommonResponse<GetProductByIdResponse> FetchProductImageById(GetProductByIdRequest objRequest)
        {
            CommonResponse<GetProductByIdResponse> response = new CommonResponse<GetProductByIdResponse>();
            ...
            try
            {
                ...
                DataSet ds = new DataSet();
                string[] TableName = { "Response", "Records" };
                _dataAccessClass.ExecuteQuery(CommandType.StoredProcedure, "FetchProductImage", ds, TableName, parameters);
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    response.ResponseCode = Convert.ToInt32(ds.Tables[0].Rows[0]["ResponseCode"]);
                    response.ResponseMessage = Convert.ToString(ds.Tables[0].Rows[0]["ResponseMessage"]);
