...
namespace Setup.BL.User.order
{
    public class CreateOrder : ICreateOrder
    {
        ...
        #region Update Payment ID Method
        public CommonResponse<UpdatePaymentResponse> UpdatePaymentId(UpdatePaymentRequest Request)
        {
            CommonResponse<UpdatePaymentResponse> response = new CommonResponse<UpdatePaymentResponse>();
            ...
            try
            {
                ...
                MySqlParameter[] parameters =
                {
                    ...
                    new MySqlParameter("@SPPaymentId", MySqlDbType.String) {Value = Request.PaymentID }
                };
                #endregion

                DataSet ds = new DataSet();
                string[] TableName = { "Response", "Records" };
                _dataAccessClass.ExecuteQuery(CommandType.StoredProcedure, "UpdatePaymentId", ds, TableName, parameters);
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    response.ResponseCode = Convert.ToInt32(ds.Tables[0].Rows[0]["ResponseCode"]);
                    response.ResponseMessage = Convert.ToString(ds.Tables[0].Rows[0]["ResponseMessage"]);
