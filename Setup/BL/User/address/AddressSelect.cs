...
namespace Setup.BL.User.address
{
    public class AddressSelect : IAddressSelect
    {
        ...
        #region Select All Address By User ID
        ...
        #region Select User Default Address By User ID
        public CommonResponse<SelectDefaultAddressResponse> GetDefaultAddress(SelectDefaultAddressRequest Request)
        {
            CommonResponse<SelectDefaultAddressResponse> response = new CommonResponse<SelectDefaultAddressResponse>();
            ...
            try
            {
                ...
                DataSet ds = new DataSet();
                string[] TableName = { "Response", "Records" };
                _dataAccessClass.ExecuteQuery(CommandType.StoredProcedure, "SelectDefaultUserAddress", ds, TableName, parameters);
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    response.ResponseCode = Convert.ToInt32(ds.Tables[0].Rows[0]["ResponseCode"]);
                    response.ResponseMessage = Convert.ToString(ds.Tables[0].Rows[0]["ResponseMessage"]);
